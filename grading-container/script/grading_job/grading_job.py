...
class GradingJob:
  ...
  def __init__(self, submission_id: int, grade_id: int, student_code: CodeFileInfo, 
    grading_script: GradingScript) -> None:
      self.__grade_id: int = grade_id
      self.__submission_id: int = submission_id
      self.__student_code: CodeFileInfo = student_code
      self.__grading_script: GradingScript = grading_script
      self.__starter_code: CodeFileInfo = None
      self.__professor_code: CodeFileInfo = None
  ...
  def retrieve_job_files(self, retrieval_cmd_factory: FileRetrievalCommandFactory, 
    ...
    student_rfc = retrieval_cmd_factory.get_file_retrieval_command(self.__student_code)
    student_rfc.execute()
    ...
    if self.__starter_code and self.__professor_code:
      ...
      starter_rfc = retrieval_cmd_factory.get_file_retrieval_command(self.__starter_code)
      starter_rfc.execute()
      ...
      professor_rfc = retrieval_cmd_factory.get_file_retrieval_command(self.__professor_code)
  ...
  def get_student_code(self) -> CodeFileInfo:
    return self.__student_code

  def get_grading_script(self) -> GradingScript:
    return self.__grading_script

  def get_starter_code(self) -> CodeFileInfo:
    return self.__starter_code

  def set_starter_code(self, starter_code: CodeFileInfo) -> None:
    self.__starter_code = starter_code

  def get_professor_code(self) -> CodeFileInfo:
    return self.__professor_code

  def set_professor_code(self, professor_code: CodeFileInfo) -> None:
