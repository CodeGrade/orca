...
from grading_job.build_script.json_helpers.grading_script_command import is_test_command
...
class CycleDetector:
  ...
  def contains_cycle(json_script: List[GradingScriptCommandJSON]):
    ...
    return CycleDetector.count_number_of_strongly_connected_components(assignments) \
      != len(json_script)
  
  @staticmethod
  def cmd_has_self_ref(json_command: GradingScriptCommandJSON, index: int) -> bool:
    return (is_test_command(json_command) and CycleDetector.test_cmd_has_self_ref(json_command, index)) \
      or CycleDetector.bash_cmd_has_self_ref(json_command, index)

  @staticmethod
  def test_cmd_has_self_ref(json_command: GradingScriptCommandJSON, index: int) -> bool:
  ...
  def visit_command(json_script: List[GradingScriptCommandJSON], unvisited: Set[int], 
    ...
    if vertex in unvisited:
      ...
      if is_test_command(command):
        CycleDetector.visit_test_command(json_script, unvisited, command_stack, vertex)
      else:
        CycleDetector.visit_bash_command(json_script, unvisited, command_stack, vertex)
  
  @staticmethod
  def visit_test_command(json_script: List[GradingScriptCommandJSON], unvisited: Set[int],
    command_stack: List[int], vertex: int) -> None:
  ...
  def assign_components(json_script: List[GradingScriptCommandJSON], vertex: int, 
    ...
    if vertex not in assignments:
      ...
      if is_test_command(command):
        CycleDetector.assign_components_test_command(json_script, vertex, root, assignments)
      else:
        CycleDetector.assign_components_bash_command(json_script, vertex, root, assignments)
  
  @staticmethod
  def assign_components_test_command(json_script: List[GradingScriptCommandJSON], 
  ...
  def assign_components_bash_command(json_script: List[GradingScriptCommandJSON],
    ...
    if json_command["on_complete"] != "complete":
      complete_vertex = int(json_command["on_complete"])
