...
class GradingJob:
  ...
  def __init__(self, submission_id: int, grade_id: int, target_code: CodeFileInfo, 
    grading_script: GradingScript) -> None:
      ...
      self.__target_code: CodeFileInfo = target_code
      self.__grading_script: GradingScript = grading_script
      self.__fixture_code: CodeFileInfo = None
      self.__test_code: CodeFileInfo = None
  ...
  def execute_grading_job(self, secret: str, audit: Audit) -> GradingJobOutput:
    ...
    Executes a grading job in full with the following workflow:
      1.) Download target, as well as possibly fixture and test code.
  ...
  def retrieve_job_files(self, retrieval_cmd_factory: FileRetrievalCommandFactory, 
    ...
    """
    audit.log_details(f"Retrieving student files from {self.__target_code}...")
    student_rfc = retrieval_cmd_factory.get_file_retrieval_command(self.__target_code)
    student_rfc.execute()
    ...
    if self.__fixture_code and self.__test_code:
      # Retrieve Starter Code
      audit.log_details(f"Retrieving starter code from {self.__fixture_code}")
      starter_rfc = retrieval_cmd_factory.get_file_retrieval_command(self.__fixture_code)
      starter_rfc.execute()
      ...
      audit.log_details(f"Retrieving professor tests/code from {self.__test_code}")
      professor_rfc = retrieval_cmd_factory.get_file_retrieval_command(self.__test_code)
  ...
  def get_target_code(self) -> CodeFileInfo:
    return self.__target_code

  def get_grading_script(self) -> GradingScript:
    return self.__grading_script

  def get_fixture_code(self) -> CodeFileInfo:
    return self.__fixture_code

  def set_fixture_code(self, starter_code: CodeFileInfo) -> None:
    self.__fixture_code = starter_code

  def get_test_code(self) -> CodeFileInfo:
    return self.__test_code

  def set_test_code(self, professor_code: CodeFileInfo) -> None:
    self.__test_code = professor_code
